...
def check_linearity(df, continous_vars, dep_var):
    ...
    for var in continous_vars:
        ...
        sns.regplot(x=var, y=dep_var, data=df, scatter=False, color='red')
...
def plot_qq(model, dep_var_name=None):
    ...
    title = f"Q-Q Plot - {dep_var_name}" if dep_var_name else "Q-Q Plot - Residuals"
    plt.title(title)
    plt.show()
...
def plot_resid_vs_fitted(model, dep_var_name=None):
    ...
    title = f"Residuals vs Fitted - {dep_var_name}" if dep_var_name else "Residuals vs Fitted"
    plt.title(title)
    plt.show()
...
def check_outliers_influence(model, dep_var_name=None):
    ...
    plt.title(f"Standardized Residuals - {dep_var_name}" if dep_var_name else "Standardized Residuals")
    ...
    plt.title(f"Cook's Distance - {dep_var_name}" if dep_var_name else "Cook's Distance")
    ...
    print(f"{dep_var_name} - Observations with |standardized residual| > 3:", extreme_resid_idx)
    print(f"{dep_var_name} - Observations with Cook's distance > 4/n:", high_cooks_idx)